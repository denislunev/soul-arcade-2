...
- **GUESS** — guess the hidden daily name in 6 tries (Wordle-style). The letter count is shown up front, and each guess must be a **real name of that exact length** (random letters aren't accepted — only genuine names, like word games). An invalid guess doesn't use up a try. After each guess, letters turn **green** (right spot), **yellow** (in the name, wrong spot), or **dark** (not in the name). Solve it in 1–2 tries for a golden celebration. A new name drops every day — keep your streak alive.
